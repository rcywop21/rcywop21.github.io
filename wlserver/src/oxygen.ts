...
export const makeAddOxygenTransform = (seconds: number): Transform => (state) => {
...
export const makeRemoveOxygenTransform = (seconds: number): Transform => (state) => {
    ...
    if (oxygenUntil <= requiredOxygenUntil) {
        ...
        throw `Unsafe to remove ${Util.formatDuration(seconds * 1000)} seconds of Oxygen if player only has ${Util.formatDuration(oxygenRemaining)}.`
    }
