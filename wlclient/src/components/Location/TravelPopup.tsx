...
const TravelPopup = (props: TravelPopupProps): React.ReactElement => {
    ...
    return (
        <div style={{display: visibility}}>
            <div className="backgroundShroud" onClick={() => setVisible(false)}></div>
            <div className="travelPopup">
                <p><b>Travel</b> - Where would you like to go?</p>
                <div className="buttonContainer">
                    { locationIds.map((locationId: Locations.LocationId): React.ReactElement => {
                        const location: Locations.Location = Locations.locationsMapping[locationId];
                        return (<button key="" onClick={handleTravelClosePopup(location.id)}>{location.name}</button>)
                    }) }
                </div>
            </div>
        </div>
