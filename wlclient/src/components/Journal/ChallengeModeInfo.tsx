...
const ChallengeModeInfo = (
    ...
    const challengeModeModifier = Math.max(15, 12.5 + 2.5 * globalState.artefactsFound).toFixed(1);

    const challengeModeModifierText = (completeFinchesCode: boolean) => completeFinchesCode
        ? (<React.Fragment>
        <p>The difficulty of Challenge Mode is measured by a percentage. It starts at 15.0%. This means that when you get Oxygen, you will only get 15.0% of the normal amount of Oxygen. For example, Diving normally gives you 20 minutes of Oxygen. With Challenge Mode (15.0%), you will only get 3 minutes of Oxygen from Diving.</p>
        <p>However, powerful magical artefacts can increase this percentage. For every artefact (except the first, which has no effect) found <b>between all groups</b>, this percentage will increase by 2.5%.</p>
    ...
    <p>For example, if 5 artefacts have been found between all the groups, Challenge Mode will be at 25.0%. Then, in that situation, you will get 5 minutes of Oxygen from Diving.</p>
    <p>Currently, {globalState.artefactsFound} artefacts have been found between all teams. Hence, you will get <span className="percentage">{challengeModeModifier}%</span> of the normal amount of Oxygen if you are in Challenge Mode.</p>
</React.Fragment>)
        : <p>You have not found out how exactly does Challenge Mode affect your oxygen gain. Perhaps it will be written down in some ancient tome? </p>

    const currentlyInChallengeModeText = playerState.challengeMode && (
        <React.Fragment>
            <h3 className="subtitle challengeColor">
                You are currently in Challenge Mode{completeFinchesCode && ` (${challengeModeModifier}%)`}!
            </h3>
            <p>
            ...
            {completeFinchesCode && (
                <p>
                    ...
                    teams, you will get <span className="percentage">{challengeModeModifier}%</span> of the normal amount of Oxygen if you are in Challenge Mode. {playerState.hasMap && 'You can check the Oxygen Guide for the exact amount of Oxygen you get from each Oxygen Stream.'}
                </p>
            )}
        </React.Fragment>
    );
    ...
    return (
        <div>
            ...
            <p>
                During Challenge Mode, you will receive much less Oxygen from diving and from Oxygen Streams than stated. If you
